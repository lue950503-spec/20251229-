...
let problem1MovingImg = null; // 新增：問題1的動畫圖片
const PROBLEM1_MOVING_FRAMES = 3;
let problem1MovingFrameIndex = 0;
let problem1MovingLastFrameTime = 0;
const PROBLEM1_MOVING_CYCLE_DURATION = 1000; // 1秒播放一次完整循環

let problem3AttackImg = null;
const PROBLEM3_ATTACK_FRAMES = 3;
let problem3AttackFrameIndex = 0;
let problem3AttackLastFrameTime = 0;
const PROBLEM3_ATTACK_CYCLE_DURATION = 1200; // 1.2秒播放一次完整循環
...
let problem2Facing = 1; // 1 = face right, -1 = face left

let problem2AttackImg = null;
const PROBLEM2_ATTACK_FRAMES = 6;
let problem2AttackFrameIndex = 0;
let problem2AttackLastFrameTime = 0;
const PROBLEM2_ATTACK_CYCLE_DURATION = 1500; // 1.5秒播放一次完整循環
...
function preload() {
  ...
  problem1MovingImg = loadImage('角色/問題一攻擊.png'); // 載入新的問題1動畫圖
...
function setup() {
  ...
  problem1MovingLastFrameTime = millis();
  problem3AttackLastFrameTime = millis();
  problem2AttackLastFrameTime = millis();
...
function draw() {
  ...
  if (!isProblem3Answered && bgImg !== gameOverBgImg) {
    ...
    // 增加距離判斷：只有在距離小於 300 且移動時才攻擊
    const distP3 = Math.abs(bottomX - width / 2);

    // 根據玩家移動狀態，決定要播放的動畫
    if ((currentAnimKey === 'walk' || currentAnimKey === 'run') && distP3 < 300) {
      // 玩家移動且靠近時，播放攻擊動畫
      imgToDraw = problem3AttackImg;
      if (imgToDraw) {
        const frameDuration = PROBLEM3_ATTACK_CYCLE_DURATION / PROBLEM3_ATTACK_FRAMES;
        if (millis() - problem3AttackLastFrameTime > frameDuration) {
          problem3AttackFrameIndex = (problem3AttackFrameIndex + 1) % PROBLEM3_ATTACK_FRAMES;
          problem3AttackLastFrameTime = millis();
        }
        frameIndexToUse = problem3AttackFrameIndex;
        srcW = imgToDraw.width / PROBLEM3_ATTACK_FRAMES;
        srcH = imgToDraw.height;
        dispW = srcW;
        dispH = srcH;
      }
    } else {
  ...
  if (!isProblem1Answered && bgImg !== gameOverBgImg) {
    ...
    // 若玩家移動，切換為攻擊動畫
    if ((currentAnimKey === 'walk' || currentAnimKey === 'run') && problem1MovingImg) {
      // 更新動畫幀
      const frameDuration = PROBLEM1_MOVING_CYCLE_DURATION / PROBLEM1_MOVING_FRAMES;
      if (millis() - problem1MovingLastFrameTime > frameDuration) {
        problem1MovingFrameIndex = (problem1MovingFrameIndex + 1) % PROBLEM1_MOVING_FRAMES;
        problem1MovingLastFrameTime = millis();
      }
      p1Img = problem1MovingImg;
      srcW = problem1MovingImg.width / PROBLEM1_MOVING_FRAMES;
      srcH = problem1MovingImg.height;
      sx = problem1MovingFrameIndex * srcW;
    }
  ...
  if (!isProblem2Answered && bgImg !== gameOverBgImg) {
    ...
    // 增加距離判斷
    const distP2 = Math.abs(bottomX - fixedX);

    // 判斷是否播放攻擊動畫 (當玩家移動時)
    if ((currentAnimKey === 'walk' || currentAnimKey === 'run') && problem2AttackImg && distP2 < 300) {
      // 播放攻擊動畫 (需靠近且移動)
      const frameDuration = PROBLEM2_ATTACK_CYCLE_DURATION / PROBLEM2_ATTACK_FRAMES;
      if (millis() - problem2AttackLastFrameTime > frameDuration) {
        problem2AttackFrameIndex = (problem2AttackFrameIndex + 1) % PROBLEM2_ATTACK_FRAMES;
        problem2AttackLastFrameTime = millis();
      }
      p2Img = problem2AttackImg;
      p2SrcW = p2Img.width / PROBLEM2_ATTACK_FRAMES;
      p2SrcH = p2Img.height;
      p2Sx = problem2AttackFrameIndex * p2SrcW;
      
      // 計算顯示尺寸 (放大顯示，解決縮小問題)
      const scaleFactor = 2.5; // 改為 2.5 倍
      p2DispH = p2SrcH * scaleFactor; 
      p2DispW = p2DispH * (p2SrcW / p2SrcH);
    } else if (problem2Img) {
      // 播放不動動畫
